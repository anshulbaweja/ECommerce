...
<div style="padding-top:15px; padding-bottom:35px;">
	...
	<p>
		<?php _e('Lists the activity log of every export with the option to restore from a backup or delete the logs that are no longer necessary.', 'wp-migration-duplicator'); ?>
	</p>
	<?php
	if ($total_list > 20) {
	?>
			...
			<table class="wt_mgdp_list_table" style="margin-top:5px;">
				...
				<tbody>
					...
					foreach ($zip_list as $key => $value) {
						$file_num++;
						$file_url = Wp_Migration_Duplicator_Admin::generate_backup_file_url($key);
					?>
						<tr>
